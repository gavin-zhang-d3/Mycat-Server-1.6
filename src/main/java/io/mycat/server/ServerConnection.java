...
public class ServerConnection extends FrontendConnection {
	 ...
	 * 
	 * private volatile int txIsolation; private volatile boolean autocommit;
	 * private volatile boolean preAcStates; // 上一个ac状态,默认为true private volatile
	 * boolean txInterrupted; private volatile String txInterrputMsg = ""; private
	 * long lastInsertId; private NonBlockingSession session;
	 *//**
	...
	public void execute(String sql, int type) {
		...
		// CONCAT(ROUND(SUM(DURATION)/*100,3), '%') AS `Percentage` FROM
		// INFORMATION_SCHEMA.PROFILING WHERE QUERY_ID= GROUP BY STATE ORDER BY SEQ
